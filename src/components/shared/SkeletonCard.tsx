...
export default function SkeletonCard({ variant = 'list', className }: SkeletonCardProps) {
  if (variant === 'compact') {
    return (
      <div className={cn('bg-[var(--surface)] rounded-lg p-4 animate-pulse', className)}>
    ...
    return (
      <Card variant="elevated" className={cn('animate-pulse', className)}>
        <CardContent className="p-6">
          ...
          <div className="h-4 bg-[var(--border)] rounded w-2/3 mb-4" />
          <div className="flex gap-2 mt-4">
            <div className="h-6 bg-[var(--border)] rounded-full w-16" />
...
export function SkeletonLine({ width = 'full', height = 4 }: { width?: string; height?: number }) {
  ...
  return <div className={cn(`h-${height} bg-[var(--border)] rounded animate-pulse`, widthClass)} />;
}

export function SkeletonCircle({ size = 12 }: { size?: number }) {
  return <div className={cn(`w-${size} h-${size} bg-[var(--border)] rounded-full animate-pulse`)} />;
}

export function SkeletonBox({ width = 14, height = 14, rounded = 'xl' }: { width?: number; height?: number; rounded?: string }) {
  return <div className={cn(`w-${width} h-${height} bg-[var(--border)] rounded-${rounded} animate-pulse`)} />;
}
