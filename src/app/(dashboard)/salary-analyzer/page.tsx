      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    ...
                    <select value={companySize} onChange={(e) => setCompanySize(e.target.value)} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white text-slate-900">
                ...
                <Card variant="elevated">
                  <CardContent className="p-6">
                    ...
                    <div className="relative h-8 bg-gradient-to-r from-red-200 via-amber-200 via-green-200 to-green-300 rounded-full mb-6">
                      ...
                      <div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-indigo-600 rounded-full border-2 border-white shadow" style={{ left: '50%', transform: 'translate(-50%, -50%)' }} />
                    </div>
        ...
        {activeTab === 'compare' && (
          <div className="space-y-6">
            <Card variant="elevated">
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><Scale className="h-5 w-5 text-indigo-600" />Compare Offers</CardTitle>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="flex gap-3">
                  ...
                  <Button variant="gradient" onClick={handleCompare} disabled={isComparing} leftIcon={isComparing ? <Loader2 className="h-4 w-4 animate-spin" /> : <Scale className="h-4 w-4" />}>{isComparing ? 'Comparing...' : 'Compare Offers'}</Button>
                </div>
              </CardContent>
            </Card>
            {comparison && (
              <div className="space-y-6">
                <Card variant="elevated"><CardContent className="p-6"><h3 className="text-lg font-semibold text-slate-900 mb-4">Total Compensation</h3><div className="space-y-3">{comparison.totalCompensation?.map((tc: any, i: number) => (<div key={i} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl"><span className="font-medium text-slate-900">{tc.company}</span><span className="text-lg font-bold text-indigo-600">{formatCurrency(tc.estimated)}</span></div>))}</div></CardContent></Card>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {comparison.recommendation && (
                  <Card variant="elevated" className="border-2 border-indigo-200 bg-indigo-50/50">
                    <CardContent className="p-6">
                      ...
                      <div className="p-4 bg-white rounded-xl"><p className="text-sm font-medium text-indigo-600 mb-1">Overall Pick: {comparison.recommendation.overallPick}</p><p className="text-slate-700">{comparison.recommendation.reasoning}</p></div>
                    </CardContent>
                  </Card>
                )}
              </div>
        ...
        {activeTab === 'negotiate' && (
          <div className="space-y-6">
            <Card variant="elevated">
              <CardHeader>
                <CardTitle className="flex items-center gap-2"><MessageSquare className="h-5 w-5 text-indigo-600" />Negotiation Script Generator</CardTitle>
                ...
                <Card variant="elevated">
                  ...
                  {expandedSections.has('opening') && (
                    <CardContent className="p-6 pt-0">
                      ...
                      <p className="text-slate-700 bg-indigo-50 p-4 rounded-xl border border-indigo-200">{script.openingStatement}</p>
                    </CardContent>
                  )}
                </Card>
                    ...
                    {expandedSections.has('keyPoints') && (
                      <CardContent className="p-6 pt-0 space-y-3">
                        {script.keyPoints.map((kp: any, i: number) => (
                          <div key={i} className="p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-200">
                            <p className="font-medium text-indigo-900">{kp.point}</p>
                            <p className="text-sm text-indigo-700 mt-1">{kp.elaboration}</p>
                            {kp.exactScript && (<div className="mt-2 p-3 bg-white rounded-lg border border-indigo-100"><p className="text-xs text-slate-500 mb-1">Say this:</p><p className="text-sm text-slate-700 italic">"{kp.exactScript}"</p></div>)}
                          </div>
                        ))}
                      </CardContent>
                    )}
                ...
                {script.timeline && (
                  <Card variant="elevated">
                    ...
                    {expandedSections.has('timeline') && (
                      <CardContent className="p-6 pt-0">
                        <div className="space-y-4">
                          {script.timeline.whenToNegotiate && (<div className="p-4 bg-indigo-50 rounded-xl border border-indigo-200"><p className="font-medium text-indigo-900 mb-1">When to Negotiate</p><p className="text-sm text-indigo-800">{script.timeline.whenToNegotiate}</p></div>)}
